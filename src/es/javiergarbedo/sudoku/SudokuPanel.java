        ...
        if (sudoku != null && !Beans.isDesignTime()) {
            ...
            for (int i = 0; i <= NUM_ROWS; i++) {
                for (int j = 0; j <= NUM_COLUMNS; j++) {
                    if (getRowSelected() == i && getColSelected() == j) {
                        g.setColor(BACKGROUND_SELECTED);
                    } else {
                        g.setColor(BACKGROUND_NORMAL);
                    }
                    g.fillRect(CELL_SIZE * j + 1, CELL_SIZE * i + 1, CELL_SIZE - 2, CELL_SIZE - 2);
                }
            }
            //TODO: Pintar los valores
            for (int i = 0; i < NUM_ROWS; i++) {
                for (int j = 0; j < NUM_COLUMNS; j++) {
                    Font a = new Font("SansSerif", BOLD, 27);
                    g.setFont(a);
                    g.setColor(Color.BLACK);
                    g.drawString(String.valueOf(sudoku.getValue(i, j)), CELL_SIZE * j + 15, CELL_SIZE * i + 30);
                }
            }
        }
    }
        ...
        if (getRowSelected() != -1 && getColSelected() != -1) {
            if (evt.getKeyChar() >= '0' && evt.getKeyChar() <= '9') {
                ...
                if (sudoku.isFinished()) {
                    ...
                    mybundle = java.util.ResourceBundle.getBundle("lang/strings");
                    String msgLevelCompleted = mybundle.getString("nivelCompletado");
